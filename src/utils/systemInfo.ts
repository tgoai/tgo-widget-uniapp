...
function parseBrowser(ua: string): string | null {
  try {
    ...
    if (mCriOS)
      return `Chrome iOS ${mCriOS[1]}`

    // iOS Firefox
    const mFxiOS = ua.match(/FxiOS\/(\d+(?:\.\d+)?)/)
    if (mFxiOS)
      return `Firefox iOS ${mFxiOS[1]}`
...
function parseOS(ua: string): string | null {
  try {
    ...
    if (mIOS)
      return `iOS ${mIOS[2].replace(/_/g, '.')}`

    // Android
    const mAndroid = ua.match(/Android (\d+(?:\.\d+)?)/)
    if (mAndroid)
      return `Android ${mAndroid[1]}`
    ...
    if (mMac)
      return `macOS ${mMac[1].replace(/_/g, '.')}`
