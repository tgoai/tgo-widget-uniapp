...
export class WuKongIMService {
  ...
  static get instance() {
    if (!this._instance)
      this._instance = new WuKongIMService()
    return this._instance
  }

  private _inited = false
  private _im: WKIM | null = null
  ...
  async init(opts: IMInitOptions) {
    ...
    if (this._im && this._bound) {
      ...
      try { this._unbindInternalEvents() }
      catch {}
    }
  ...
  private _unbindInternalEvents() {
    if (!this._im || !this._bound)
      return
    if (this._hConnect)
      this._im.off(WKIMEvent.Connect, this._hConnect as any)
    if (this._hDisconnect)
      this._im.off(WKIMEvent.Disconnect, this._hDisconnect as any)
    if (this._hError)
      this._im.off(WKIMEvent.Error, this._hError as any)
    if (this._hMessage)
      this._im.off(WKIMEvent.Message, this._hMessage as any)
    if (this._hCustom)
      this._im.off(WKIMEvent.CustomEvent, this._hCustom as any)
    this._hConnect = this._hDisconnect = this._hError = this._hMessage = this._hCustom = undefined
    this._bound = false
  }

  private _emitMessage(m: RecvMessage) {
  ...
  private _emitStatus(s: IMStatus, info?: any) {
  ...
  private _emitCustom(e: any) {
    this._customListeners.forEach((fn) => {
      try { fn(e) }
      catch {}
    })
  }
  ...
  async connect() {
    if (!this._im)
      throw new Error('WuKongIMService not initialized')
  ...
  private async _fetchRouteWsAddr(apiBase: string, uid: string, timeoutMs = 10000): Promise<string> {
    ...
    // const url = new URL('v1/wukongim/route', base)
    // url.searchParams.set('uid', uid)
    const controller = new AbortController()
    const timer = setTimeout(() => controller.abort(), timeoutMs)
    try {
      const res = await un.get(`${base}v1/wukongim/route`, {
        signal: controller.signal,
        params: { uid },
      })
      console.log('[WuKongIM] Route response:', res)
      if (res.status !== 200) {
        // throw new Error(`route HTTP ${res.status} ${res.errMsg}`)
      }
      ...
      if (!addr && isHttps)
        addr = data?.wss ?? data?.ws_addr_tls
      if (!addr || typeof addr !== 'string')
        throw new Error('missing ws address')
      let wsAddr = String(addr)
      if (/^https?:/i.test(wsAddr))
        wsAddr = wsAddr.replace(/^http/i, 'ws')
      return wsAddr
    }
    catch (e: any) {
  ...
  async sendText(text: string, opts?: { to?: string, channelType?: 251, clientMsgNo?: string, header?: any }) {
    if (!this._im || !this._cfg)
      throw new Error('WuKongIMService not ready')
