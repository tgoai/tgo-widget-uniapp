import type { CachedVisitor, VisitorRegisterRequest, VisitorRegisterResponse } from '@/types/api'
...
export async function registerVisitor(params: {
  ...
  try {
    const body: VisitorRegisterRequest = {
      platform_api_key: platformApiKey,
      ...extra,
    }
    ...
    if (res.status !== 200) {
      // throw new Error(`[Visitor] register failed: ${res.status} ${res.errMsg}`)
    }
    const data = res.data as VisitorRegisterResponse
